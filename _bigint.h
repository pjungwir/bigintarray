...
/* Compat: palloc_object/palloc_array added in PG16 */
#ifndef palloc_object
...
/* INT8ARRAYOID should be in pg_type_d.h, but define fallback */
#ifndef INT8ARRAYOID
#define INT8ARRAYOID 1016
#endif

/* number ranges for compression */
#define G_BIGINT_NUMRANGES_DEFAULT		100
...
} GISTBigintArrayOptions;

/* useful macros for accessing int8 arrays */
...
/* Hash int64 values by mixing high and low 32 bits */
#define HASHVAL(val, siglen) (((unsigned int)((int32)(val) ^ (int32)((val) >> 32))) % SIGLENBIT(siglen))
#define HASH(sign, val, siglen) SETBIT((sign), HASHVAL(val, siglen))

/* gist__bigintbig_ops opclass options */
typedef struct
{
	int32		vl_len_;		/* varlena header (do not touch directly!) */
	int			siglen;			/* signature length in bytes */
} GISTBigintArrayBigOptions;
 ...
 * useful functions
 */
void		isort(int64 *a, size_t len, bool ascending);
ArrayType  *new_intArrayType(int num);
ArrayType  *copy_intArrayType(ArrayType *a);
ArrayType  *resize_intArrayType(ArrayType *a, int num);
int			internal_size(int64 *a, int len);
ArrayType  *_int_unique(ArrayType *r);
int32		intarray_match_first(ArrayType *a, int64 elem);
ArrayType  *intarray_add_elem(ArrayType *a, int64 elem);
ArrayType  *intarray_concat_arrays(ArrayType *a, ArrayType *b);
ArrayType  *int_to_intset(int64 elem);
bool		inner_int_overlap(ArrayType *a, ArrayType *b);
bool		inner_int_contains(ArrayType *a, ArrayType *b);
ArrayType  *inner_int_union(ArrayType *a, ArrayType *b);
ArrayType  *inner_int_inter(ArrayType *a, ArrayType *b);
void		rt__int_size(ArrayType *a, float *size);
void		gensign(BITVECP sign, int64 *a, int len, int siglen);
...
#define QSORT(a, direction) \
	do { \
		...
		isort(ARRPTR(a), _nelems_, (direction) ? true : false); \
	} while(0)
