 ...
 */
static int32
gettoken(WORKSTATE *state, int64 *val)
{
	char		nnn[24];
	int			innn;
		...
		switch (state->state)
		{
			...
			case WAITENDOPERAND:
				...
				else
				{
					...
					nnn[innn] = '\0';
					errno = 0;
					llval = strtoll(nnn, &endptr, 10);
					*val = (int64) llval;
					if (errno != 0 || endptr == nnn || (long long) *val != llval)
						return ERR;
 ...
 */
static void
pushquery(WORKSTATE *state, int32 type, int64 val)
{
	...
	while ((type = gettoken(state, &val)) != END)
	{
		switch (type)
		{
			case VAL:
				...
				while (lenstack && (stack[lenstack - 1] == (int64) '&' ||
									stack[lenstack - 1] == (int64) '!'))
				{
					lenstack--;
...
{
	int64	   *StopLow = ((CHKVAL *) checkval)->arrb;
	int64	   *StopHigh = ((CHKVAL *) checkval)->arre;
	int64	   *StopMiddle;
...
gin_bool_consistent(QUERYTYPE *query, bool *check)
{
	 ...
	 * extraction code in ginint8_queryextract.
	 */
	gcv.first = items;
...
findoprnd(ITEM *ptr, int32 *pos)
{
	...
	elog(DEBUG3, (ptr[*pos].type == OPR) ?
		 "%d  %c" : "%d  " INT64_FORMAT, *pos, ptr[*pos].val);
#endif
	if (ptr[*pos].type == VAL)
	{
		ptr[*pos].left = 0;
...
bqarr_in(PG_FUNCTION_ARGS)
{
	...
	{
		if (ptr[i].type == OPR)
			appendStringInfo(&pbuf, "%c(%d) ", (char) ptr[i].val, ptr[i].left);
		else
			appendStringInfo(&pbuf, INT64_FORMAT " ", ptr[i].val);
	}
...
infix(INFIX *in, bool first)
{
	...
	{
		RESIZEBUF(in, 24);
		sprintf(in->cur, INT64_FORMAT, in->curpol->val);
		in->cur = strchr(in->cur, '\0');
		in->curpol--;
	}
	else if (in->curpol->val == (int64) '!')
	{
		bool		isopr = false;
