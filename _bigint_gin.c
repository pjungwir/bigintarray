...
PG_FUNCTION_INFO_V1(ginint8_queryextract);

Datum
ginint8_queryextract(PG_FUNCTION_ARGS)
{
	...
	if (strategy == BooleanSearchStrategy)
	{
		...
		*nentries = 0;
		...
		for (i = 0; i < query->size; i++)
		{
			if (items[i].type == VAL)
			{
				res[*nentries] = Int64GetDatum(items[i].val);
				(*nentries)++;
			}
		}
	}
		...
		*nentries = ARRNELEMS(query);
		...
		switch (strategy)
		{
			...
			case RTOldContainsStrategyNumber:
				...
				break;
			default:
				elog(ERROR, "ginint8_queryextract: unknown strategy number: %d",
					 strategy);
		}
	}
...
PG_FUNCTION_INFO_V1(ginint8_consistent);

Datum
ginint8_consistent(PG_FUNCTION_ARGS)
{
	...
	switch (strategy)
	{
		...
		case BooleanSearchStrategy:
			...
			break;
		default:
			elog(ERROR, "ginint8_consistent: unknown strategy number: %d",
				 strategy);
	}
