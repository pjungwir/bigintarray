 ...
 */
#define MAXNUMELTS (Min((MaxAllocSize / sizeof(Datum)),((MaxAllocSize - ARR_OVERHEAD_NONULLS(1)) / sizeof(int64)))/2)

/*
** GiST support methods
*/
PG_FUNCTION_INFO_V1(g_bigint_consistent);
PG_FUNCTION_INFO_V1(g_bigint_compress);
PG_FUNCTION_INFO_V1(g_bigint_decompress);
PG_FUNCTION_INFO_V1(g_bigint_penalty);
PG_FUNCTION_INFO_V1(g_bigint_picksplit);
PG_FUNCTION_INFO_V1(g_bigint_union);
PG_FUNCTION_INFO_V1(g_bigint_same);
#if PG_VERSION_NUM >= 130000
PG_FUNCTION_INFO_V1(g_bigint_options);
#endif


/*
** The GiST Consistent method for bigint arrays
*/
Datum
g_bigint_consistent(PG_FUNCTION_ARGS)
{
	...
	*recheck = (strategy == RTSameStrategyNumber);
	...
	switch (strategy)
	{
		...
		case RTSameStrategyNumber:
			if (GIST_LEAF(entry))
				DirectFunctionCall3(g_bigint_same,
									entry->key,
		...
		case RTOldContainedByStrategyNumber:
			...
			/*
			 * This code is unreachable as of bigintarray 1.0, because the <@
			 * operator has been removed from the opclass.  We keep it for now
			 * to support possible future versions.
			 */
			if (GIST_LEAF(entry))
...
Datum
g_bigint_union(PG_FUNCTION_ARGS)
{
	...
	for (i = 0; i < entryvec->n; i++)
	{
		...
		nel = ARRNELEMS(ent);
		memcpy(ptr, ARRPTR(ent), nel * sizeof(int64));
		ptr += nel;
	}
...
*/
Datum
g_bigint_compress(PG_FUNCTION_ARGS)
{
	GISTENTRY  *entry = (GISTENTRY *) PG_GETARG_POINTER(0);
	GISTENTRY  *retval;
	ArrayType  *r;
	int			num_ranges = G_BIGINT_GET_NUMRANGES();
	int			len,
				lenr;
	int64	   *dr;
	int			i,
		...
		if (ARRNELEMS(r) >= 2 * num_ranges)
			ereport(ERROR,
					(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
					 errmsg("input array is too big (%d maximum allowed, %d current), use gist__bigintbig_ops opclass instead",
	...
	{							/* compress */
		...
		if (++j)
		{
			 ...
			 */
			memmove(&dr[0], &dr[2 * j], 2 * (len - j) * sizeof(int64));
		}
		...
		while (len > num_ranges * 2)
		{
			...
			memmove(&dr[cand - 1], &dr[cand + 1], (len - cand - 1) * sizeof(int64));
			len -= 2;
		}
		...
		if (lenr < 0 || lenr > MAXNUMELTS)
			ereport(ERROR,
					(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
					 errmsg("data is too sparse, recreate index using gist__bigintbig_ops opclass instead")));
...
Datum
g_bigint_decompress(PG_FUNCTION_ARGS)
{
	GISTENTRY  *entry = (GISTENTRY *) PG_GETARG_POINTER(0);
	GISTENTRY  *retval;
	ArrayType  *r;
	...
	if (lenr < 0 || lenr > MAXNUMELTS)
		ereport(ERROR,
				(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
				 errmsg("compressed array is too big, recreate index using gist__bigintbig_ops opclass instead")));
...
/*
** The GiST Penalty method for bigint arrays
*/
Datum
g_bigint_penalty(PG_FUNCTION_ARGS)
{
...
Datum
g_bigint_same(PG_FUNCTION_ARGS)
{
...
/*
** The GiST PickSplit method for bigint arrays
** We use Guttman's poly time split algorithm
*/
Datum
g_bigint_picksplit(PG_FUNCTION_ARGS)
{
	GistEntryVector *entryvec = (GistEntryVector *) PG_GETARG_POINTER(0);
...
#if PG_VERSION_NUM >= 130000
Datum
g_bigint_options(PG_FUNCTION_ARGS)
{
	local_relopts *relopts = (local_relopts *) PG_GETARG_POINTER(0);

	init_local_reloptions(relopts, sizeof(GISTBigintArrayOptions));
	add_local_int_reloption(relopts, "numranges",
							"number of ranges for compression",
							G_BIGINT_NUMRANGES_DEFAULT, 1, G_BIGINT_NUMRANGES_MAX,
							offsetof(GISTBigintArrayOptions, num_ranges));
