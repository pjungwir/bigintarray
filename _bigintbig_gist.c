...
** _bigintbig methods
*/
PG_FUNCTION_INFO_V1(g_bigintbig_consistent);
PG_FUNCTION_INFO_V1(g_bigintbig_compress);
PG_FUNCTION_INFO_V1(g_bigintbig_decompress);
PG_FUNCTION_INFO_V1(g_bigintbig_penalty);
...
Datum
_bigintbig_in(PG_FUNCTION_ARGS)
{
	ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("cannot accept a value of type %s", "bigintbig_gkey")));

	PG_RETURN_VOID();			/* keep compiler quiet */
}

Datum
_bigintbig_out(PG_FUNCTION_ARGS)
{
	ereport(ERROR,
			(errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
			 errmsg("cannot display a value of type %s", "bigintbig_gkey")));
...
Datum
g_bigintbig_same(PG_FUNCTION_ARGS)
{
...
Datum
g_bigintbig_compress(PG_FUNCTION_ARGS)
{
...
sizebitvec(BITVECP sign, int siglen)
{
	...
	/* Simple popcount implementation for portability */
	LOOPBYTE(siglen)
	{
		unsigned char byte = (unsigned char) sign[i];
...
Datum
g_bigintbig_decompress(PG_FUNCTION_ARGS)
{
...
Datum
g_bigintbig_union(PG_FUNCTION_ARGS)
{
...
Datum
g_bigintbig_penalty(PG_FUNCTION_ARGS)
{
...
{
	int32		ca = ((const SPLITCOST *) a)->cost;
	int32		cb = ((const SPLITCOST *) b)->cost;
...
Datum
g_bigintbig_picksplit(PG_FUNCTION_ARGS)
{
	GistEntryVector *entryvec = (GistEntryVector *) PG_GETARG_POINTER(0);
...
Datum
g_bigintbig_consistent(PG_FUNCTION_ARGS)
{
	GISTENTRY  *entry = (GISTENTRY *) PG_GETARG_POINTER(0);
...
g_bigintbig_options(PG_FUNCTION_ARGS)
{
	...
	init_local_reloptions(relopts, sizeof(GISTBigintArrayBigOptions));
	add_local_int_reloption(relopts, "siglen",
							"signature length in bytes",
							SIGLEN_DEFAULT, 1, SIGLEN_MAX,
							offsetof(GISTBigintArrayBigOptions, siglen));
